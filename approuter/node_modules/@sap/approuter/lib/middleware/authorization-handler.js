...
module.exports = async function checkAuthorization(req) {
  ...
  const auth = req.headers && req.headers['x-approuter-authorization'];
  if (!auth) {
    const tenantInfo = getTenantInfo(req);
    // service to approuter middleware, the flow is different,
    // tenant url might be different from the token tenant,
    // but it doesn't mean the user is not authorized,
    // so skip tenant check in this case
