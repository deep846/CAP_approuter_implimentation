...
exports.processCookies = function (setCookieHeader, logger) {
  let removeClashingBackendSessionCookie = process.env.REMOVE_CLASHING_BACKEND_SESSION_COOKIE;
  let aprSessionCookieName = cookieUtils.getSessionCookieName();
  let sessionCookies = [];
  let nonSessionCookies = [];
  if (setCookieHeader) {
    for (let i = 0; i < setCookieHeader.length; ++i) {
      let oCookie = tough.Cookie.parse(setCookieHeader[i]);
      if (oCookie && isSessionCookie(oCookie, removeClashingBackendSessionCookie, aprSessionCookieName,logger)) {
...
exports.storeSessionCookies = function(sessionCookies, req, res) {
  ...
  const storeSessionCookiesInExternalSessionStore = req && req.routerConfig && req.routerConfig.storeSessionCookiesInExternalSessionStore;
  if (isService2Approuter && res && !storeSessionCookiesInExternalSessionStore) {
...
function isSessionCookie(oCookie, removeClashingBackendSessionCookie, aprSessionCookieName, logger) {
  if ((removeClashingBackendSessionCookie === 'true') && aprSessionCookieName && oCookie.key === aprSessionCookieName) {
    logger.info(`Backend cookie ${oCookie.key} (domain: ${oCookie.domain || 'N/A'}) is identified as approuter session cookie`);
    return true;
  }
