...
const iasUtils = require('./lib/utils/ias-utils');
...
Approuter.prototype.start = async function (options, callback) {
  let self = this;
  ...
  app.logger = logger;
  await enrichStartupData(app, logger);
  app.approuter = this;
...
async function enrichStartupData(app, logger) {
  const iasConfig = app.get('mainRouterConfig').iasConfig || {};
  const iasCredentials = iasConfig.options || {};
  if (!Object.keys(iasCredentials).length) {
    return;
  }
  let missing;
  if (!iasCredentials.app_tid) {
    missing = 'app_tid missing';
  } else if (!iasCredentials['btp-tenant-api']) {
    missing = 'btp-tenant-api missing';
  }
  if (missing) {
    logger.error(`Skipping IAS identityzone population at startup: ${missing}`);
    return;
  }

  try {
    iasCredentials.identityzone = await iasUtils.getSubdomainByApptid(iasCredentials.app_tid, iasCredentials['btp-tenant-api']);
    logger.info('IAS identityzone populated at startup: %s', iasCredentials.identityzone);
  } catch (err) {
    logger.error('Failed to populate IAS identityzone at startup: %s', err.message);
  }
}
...
function addImplicitExtension(ar, options) {
  let ext = {
    insertMiddleware: {
      ...
      firstWS: ar.firstWS._middleware,
      beforeRequestHandlerWS: ar.beforeRequestHandlerWS._middleware
    }
  };
